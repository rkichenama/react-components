  ...
  return (
    <ForecastDeck className='hourly'>
      {time.slice(offset, offset + cardCount).map((stamp, i) => {
        ...
        return (
          <Card key={stamp} img={Img} temp={Math.round(temperature2m[ index ])} desc={showDescription ? (
            <>
...
export const DailyForecast = ({ daily: { time, temperature2mMax, temperature2mMin, weathercode } }: OpenMeteoResponse) => {
  const { showDescription, cardCount } = React.useContext(WeatherContext);
  ...
  return (
    <ForecastDeck className='daily'>
      {time.slice(1, cardCount + 1).map((stamp, i) => {
        ...
        return (
          <Card key={stamp} img={Img} temp={`${Math.round(temperature2mMax[ index ])}/${Math.round(temperature2mMin[ index ])}`} desc={showDescription ? (
            <>
